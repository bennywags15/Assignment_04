...
airport_smry_df %>%
  ...
  labs(x = NULL,
       y = NULL,
       title = "Airports which had the largest monthly average % of flights that were more than 20 minutes late from 2010-2017") +
  theme_classic()
```
Graph 3:
...
month_smry <-
  ...
  summarize(prop_late_over20 = mean(arr_delay > 20),
            num_flights = n(),
            avg_len_flight = mean(air_time),
            num_cancellations = count(cancelled),
            avg_dist_flight = mean(distance))
...
`month`,
  ...
  COUNT(*) AS `num_flights`,
  AVG(`air_time`) AS `avg_len_flight`,
  count(`cancelled`) AS `num_cancellations`,
  AVG(`distance`) AS `avg_dist_flight`
FROM `flights`
WHERE (`year` >= 2017.0)
GROUP BY `month`
